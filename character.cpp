            ...
            if(lastpoisondamage != -1)
            {
                ...
                { 
                    ...
                    if((MaxHealth() * (float)pcnt / 100.0f) >= 1)
                        SetHealth(int(Health() - max(1,(MaxHealth() * (float)pcnt / 100.0f))));
                    else
                        SetHealth(1);
                }
            }
    ...
    for (int move = 0; move < nummoves; move++)
    {
      ...
      // Check to see if character is blocked
        ...
        else
        {
            ...
            if (newpos.z != nh)
                if (vel.z < 0)
                    newpos.z = (int)(nh * CHARMOVE_MULTIPLIER);
                else
                    newpos.z += (int)(((nh - newpos.z) / 2) * CHARMOVE_MULTIPLIER);
      ...
      // We've done what we could to go forward... now..
        ...
        else            // Set new position add add accumulator
        {
            ...
            if (newpos.z < (MapPane.GetWalkHeight(pos) * CHARMOVE_MULTIPLIER))
                newpos.z = (int)(MapPane.GetWalkHeight(pos) * CHARMOVE_MULTIPLIER);
        ...
        else if (doing->action == GetMoveAction(root->action))
        {
            if (Fighting())
            {
                ...
                else
                {
                    ...
                    {
                        int needed_angle = AngleTo(Fighting());
...
//                      else
                    ...
                    else
                    {
                        ...
                        {
                            int needed_angle = AngleTo(Fighting());
                            int target_angle = ConvertToFacing(pos, target_last_position) + 63;
...
BOOL TCharacter::FindButtonAttack(int id, int dmgpcnt, int &attacknum, int &impactnum, int &damage, BOOL isaction)
{
    if (!IsFighting())
        return FALSE;
...
BOOL TCharacter::ButtonAction(int buttonid)
{
    int dmgpcnt = random(1, 50) + random(1, 50);
    int attacknum, impactnum, damage;
