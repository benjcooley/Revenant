...
// Load from wave file
PTSound TSound::Load(char *name, int dirresid)
{
...
#define SOUND_RAMPLEVELS    (32)    
    if (!looping)
    {
            ...
            sound->size > 64)
        {
            ...
            for (int c = 0; c < SOUND_RAMPLEVELS; c++, s++, e--)
            {
            ...
            sound->size > 32)
        {
            ...
            for (int c = 0; c < SOUND_RAMPLEVELS; c++, s++, e--)
            {
                *s = (BYTE)(*s * (float)(c / SOUND_RAMPLEVELS)); 
                *e = (BYTE)(*e * (float)(c / SOUND_RAMPLEVELS)); 
            }
        }
    }
#endif
...
int CalcPan(PS3DPoint lpos, PS3DPoint spos)
{
    int pan = 0;
    S3DPoint tmp_pos;
    ...
    if (lpos && spos)
    {
        int distance = ::Distance(*lpos, *spos);
        ...
        if (distance < MAXDIST)
        {
            ...
            a += (M_PI / 2.0);
            pan = (int)(cos(a) * (double)DSBPAN_RIGHT / 16);
        }
    }
...
// lpos is the listeners position
// spos is the sound's position
int CalcDirectionalVol(int orig_vol, PS3DPoint lpos, PS3DPoint spos)
{
    int vol = orig_vol;
...
// lpos is the listeners position
// spos is the sound's position
void TSound::Play(int volume, int freq, PS3DPoint lpos, PS3DPoint spos)
{
    int pan = 0;
...
BOOL TSoundPlayer::Play(int id, int volume, int freq, PS3DPoint spos)
{
    if (!Functioning() || id < 0 || id >= soundlist.NumItems())
        return FALSE;
...
void TSoundPlayer::SetListenerPos(int x, int y, int z)
{
    PSSoundRef ref;
...
BOOL TSoundPlayer::SearchSoundDir(char *soundpath, char *subdir, int dirresid)
{
    char fname[MAX_PATH];
