			...
			else
			{
			  ...
			  // Current motion data includes vert, and rotx,roty,rotz
 				if (mesh->flags & I3D_ROOTMOTION)
					memcpy(motion[c], mesh->statedata[c].motion.ptr(), 
						sizeof(SMotionData) * GetAniLength(c));
				else if (mesh->flags & I3D_FACINGMOTION) // Old facing data (no vert,rotx,roty)
				{
				  // Convert old motion data format to new format
					PSOldMotionData omd = (PSOldMotionData)(mesh->statedata[c].motion.ptr());
					PSMotionData nmd = motion[c];
					for (int d = 0; d < GetAniLength(c); d++, omd++, nmd++)
					{
						nmd->dist = omd->dist;
				...
				{
				  // Convert old motion data format to new format
					PSOldOldMotionData omd = (PSOldOldMotionData)(mesh->statedata[c].motion.ptr());
					PSMotionData nmd = motion[c];
					for (int d = 0; d < GetAniLength(c); d++, omd++, nmd++)
					{
	...
	D3DVECTOR &pos, D3DVECTOR &rot, D3DVECTOR &scl)
{
	...
	if (!Interpolate ||													// Interpolation off
		(state == prevstate && GetHeader()->states[state].aniflags & AF_LOOPING) || // This is a looping state
...
void T3DImagery::SetObjectMotion(PTObjectInstance inst)
{
		...
		(frame != oldframe || state != oldstate))
	{
		...
		if (motion)
		{
			...
			movement.z = vert;				// Add vertical movement
			inst->SetNextMove(movement);	// Set next move for object
