...
static bool UseBlue = false;	// switch to activate Revenant-Blue interface module 
...
BOOL T3DScene::Initialize()
{
	...
	//>>>>>>>>>>>>>>>>>>>>>>
	UsingHardware = FALSE;		// THIS LINE IS INSERTED TO TEST REVENANT-BLUE INTERFACE MODULE 
								//  --------- TO BE DELETED !!!!!!!! ---------
	if (!UsingHardware) {
		UseBlue = true;
...
{
	//>>>>>>>>>>>>>>>>>>
	// Scale object extent here since the projection matrix 
	// is not passed to D3D under Blue.
	if (UseBlue) {
		int delta = (r.right-r.left)/2;
		r.left -= delta;
		r.right += delta;
		delta = (r.bottom-r.top)/2;
		r.top -= delta;
		r.bottom += delta;
	}
	//<<<<<<<<<<<<<<<<<<
