...
//	}
	...
	/*
	 * Beginning and ending pose names for each state
	 */

	begstate = new char*[NumStates()];
	memset(begstate, 0, sizeof(char*) * NumStates());
	endstate = new char*[NumStates()];
	memset(endstate, 0, sizeof(char*) * NumStates());
...
//	}
  ...
  // Go through all objects
	...
	if (mesh->version >= 2)
	{
		for (c = 0; c < NumStates(); c++)
		{
			PS3DImageryState ist = &(mesh->statedata[c]);
		
			if (ist->begstate[0])
				begstate[c] = _strdup(ist->begstate);
			if (ist->endstate[0])
				endstate[c] = _strdup(ist->endstate);
		}
	}
...
//			} // Mount all sounds in animation for right now
		}
	}
...
void T3DImagery::ClearMesh()
{
  ...
  // Delete and unmount sounds
	if (numplaysounds > 0)
	{
		for (int ps = 0; ps < numplaysounds; ps++)
		{
//			if (playsounds[ps].mount)	// Mount all sounds right now
//			{
				int num = listnum(playsounds[ps].sounds);  // Get number of sounds in list
				for (int n = 0; n < num; n++)
	...
	D3DVECTOR &pos, D3DVECTOR &rot, D3DVECTOR &scl)
{
	...
	if (begstate[state] && endstate[prevstate] &&					// Has begin/end state frames	
		prevframe >= (GetHeader()->states[prevstate].frames - 1) &&	// and last state went to last frame
		!stricmp(endstate[prevstate], begstate[state]))				// and beg pose for this state matches end pose of prev
		return TRUE;
	...
	for (int c = 0; c < numplaysounds; c++, ps++)
	{
		if (ps->state == state && ps->frame == frame)
		{
			if (!inst)
				...
				inst->PlayWave(listrnd(ps->sounds));
			return;
		}
	}
}
