...
void TAuraAnimator::Initialize()
{
    ...
    ((PTEffect)inst)->GetSpell()->SetDefense(((PTEffect)inst)->GetSpell()->VariantData()->mindamage);
    ((PTEffect)inst)->GetSpell()->SetOffense(((PTEffect)inst)->GetSpell()->VariantData()->maxdamage);
}
...
// added by pepper to move quicksand to the target
    ...
    if(spell)
    {
        ...
        else if(strcmp(spell->VariantData()->name, "Quicksand3") == 0)
        {
            level = 3;
        }
...
/*      PTCharacter invoker = (PTCharacter)spell->GetInvoker();
        ...
        {
            PTCharacter invoker = (PTCharacter)spell->GetInvoker();
            ((PTEffect)inst)->GetPos(temp_point);
...
*/  }
    else
    {
        num_targets = 0;
        target[0] = NULL;
        ((PTEffect)inst)->GetPos(target_position[0]);
        ((PTEffect)inst)->GetPos(temp_point);
        level = 1;
    }

    heaven_scale[0] = (float)random(50,100);
    heaven_scale[1] = (float)random(50,100);
    spell = ((PTQuicksandEffect)inst)->GetSpell();
    if(spell)
    {
        PTCharacter invoker = (PTCharacter)spell->GetInvoker();
        if(invoker)
        {
            invoker->GetPos(heaven_pos[0]);
            heaven_pos[0].z += 200;
            invoker->GetPos(heaven_pos[1]);
            heaven_pos[1].z += 200 + (int)heaven_scale[0];
        }
        else
        {
            heaven_pos[0].x = 0;
...
void TQuicksandAnimator::Animate(BOOL draw)
{
    ...
    if(frameon < 15)
    {
        ...
        if((heaven_pos[0].z + heaven_scale[0]) < 0)
        {
            heaven_pos[0].z = heaven_pos[1].z + (int)heaven_scale[1];
        }

        if((heaven_pos[1].z + heaven_scale[1]) < 0)
        {
            heaven_pos[1].z = (int)heaven_pos[0].z + (int)heaven_scale[0];
        }
    }
    else if(frameon == 15)
    {
        ...
        if((heaven_pos[0].z - heaven_scale[0]) >= 0 && (heaven_pos[1].z - heaven_scale[1]) >= 0)
        {
            frameon = 14;
        }
        else
        {
            if(num_targets > 0)
            {
                heaven_pos[0].x = target_position[0].x;
                heaven_pos[0].y = target_position[0].y;
                heaven_pos[0].z = target_position[0].z;
                heaven_pos[0].z -= (int)(10 + heaven_scale[0]);
                heaven_pos[1].x = target_position[0].x;
                heaven_pos[1].y = target_position[0].y;
                heaven_pos[1].z = target_position[0].z;
                heaven_pos[1].z -= (int)(heaven_pos[0].z + heaven_scale[1]);
            }
        }
    }
    else if(frameon < 30)
    ...
    else if(scalesize < 1.0f && frameon > 30)
    {
        scalesize += 0.05f;
    }
// added by pepper to rotate the bad, bad man
    if(frameon > QUICKSAND_SPIN_START)
    {
        ...
        {
            if(target[i]  && !target[i]->IsDead())
            {
                target[i]->SetNoCollision(TRUE);
                target[i]->SetRotateZ((int)target_rotation);
                if(!target[i]->IsFlailing())
                {
                    target[i]->Flail();
                    if(spell)
                    {
                        spell->Damage(target[i]);
                    }
                }
                target_rotation+=10;
                temp_z_level = (float)target_position[i].z;
                target[i]->GetPos(target_position[i]);
                target_position[i].z = (int)temp_z_level;
                ((PTEffect)inst)->SetPos(target_position[0]);

                // Gobble up the bad, bad man
                if(level == 3)
                {
                    z_level -= 50;
                    S3DPoint temp_point;
                    target[i]->GetPos(temp_point);
                    temp_point.z = z_level;
                    target[i]->SetPos(temp_point);
                }
            }
        }
    }
...
//  UpdateBoundingRect(&extents);
    ...
    if(frameon <= 30)
    {
        ...
        if(obj->lverts)
        {
            ...
            {
                obj->lverts[i].color = D3DRGBA(1.0f,1.0f,1.0f,0.5f);
            }
        }
        ...
        obj->scl.x = 1.0f;
        obj->scl.y = 1.0f;
        obj->scl.z = heaven_scale[0];

        obj->pos.x = (float)heaven_pos[0].x;
        obj->pos.y = (float)heaven_pos[0].y;
        obj->pos.z = (float)heaven_pos[0].z;

//      RenderObject(obj);

        obj->scl.x = 1.0f;
        obj->scl.y = 1.0f;
        obj->scl.z = heaven_scale[1];

        obj->pos.x = (float)heaven_pos[1].x;
        obj->pos.y = (float)heaven_pos[1].y;
        obj->pos.z = (float)heaven_pos[1].z;
...
void TTornadoAnimator::Animate(BOOL draw)
{
    ...
    if (frameon % TORNADO_DURATION == 0)
    {
        PTObjectInstance invoker = ((PTEffect)inst)->GetSpell()->GetInvoker();
        S3DPoint pos;
        ((PTEffect)inst)->GetPos(pos);
        DamageCharactersInRange(invoker, pos, TORNADO_MODIFIER, ((PTEffect)inst)->GetSpell()->VariantData()->mindamage, ((PTEffect)inst)->GetSpell()->VariantData()->maxdamage, ((PTEffect)inst)->GetSpell()->SpellData()->damagetype);
    }
