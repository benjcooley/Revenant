...
class TSurface
{
  ...
  // functions.
    ...
    BOOL ParamBlitSetup(RSDrawParam tmpdp, PTSurface srcsurface, int flags, LPDDBLTFX fx);
  ...
  // Low level Put and Blit which ONLY do primary surface (no Z or Normal Buffer)
    virtual BOOL BlitHandler(PSDrawParam dp, PTSurface surface, int flags = 0, LPDDBLTFX fx = NULL);
...
// --------------------------------------------------------------------------------
  ...
  // Shortcut blit functions
    BOOL Blit(int x, int y, int w, int h, 
        DWORD drawmode = DM_USEDEFAULT, int flags = 0, LPDDBLTFX fx = NULL)
    { 
        SDrawParam dp;
        MakeDPNoSrc(dp, x, y, w, h, drawmode);
        return ParamBlit(&dp, NULL, flags, fx); 
    }

  // Shortcut blit functions
    BOOL Blit(int x, int y, PTSurface surface, int sx, int sy, int swidth, 
               int sheight, DWORD drawmode = DM_USEDEFAULT, int flags = 0, LPDDBLTFX fx = NULL)
    { 
        SDrawParam dp;
        MakeDP(dp, x, y, sx, sy, swidth, sheight, drawmode);
        return ParamBlit(&dp, surface, flags, fx); 
    }
    
    BOOL Blit(int x, int y, PTSurface surface, DWORD drawmode = DM_USEDEFAULT, 
               int flags = 0, LPDDBLTFX fx = NULL)
    { 
        return Blit(x, y, surface, 0, 0, surface->Width(), surface->Height(), drawmode, flags, fx);
    }
  ...
  // Shortcut blit functions
    ...
    { 
        SDrawParam dp;
        MakeDP(dp, x, y, sx, sy, swidth, sheight, drawmode);
        return ParamGetBlit(&dp, surface, flags, fx); 
    }
    
    BOOL GetBlit(int x, int y, PTSurface surface, DWORD drawmode = DM_USEDEFAULT, 
               int flags = 0, LPDDBLTFX fx = NULL)
    { 
        return GetBlit(x, y, surface, 0, 0, surface->Width(), surface->Height(), drawmode, flags, fx);
    }
