...
struct S3DAnimObj
{
    ...
    int numfaces;
    S3DFace *faces;
    int texfaces[MAXTEXTURES + 1];
    int numtexfaces[MAXTEXTURES + 1];
    int textureframe[MAXTEXTURES + 1];
    D3DTEXTUREHANDLE htextures[MAXTEXTURES];
    D3DMATERIALHANDLE hmaterial;
};
typedef TPointerArray<S3DAnimObj, MAX3DANIMOBJECTS> T3DAnimObjArray;
...
class T3DImagery : public TObjectImagery
{
  ...
  public:
    ...
    void PlaySound(int state, int frame);
  ...
  // Miscellaneous support functions
    ...
    void DrawShadow(PTObjectInstance oi);
      // Called before rendering the object to draw the shadow for it
    void ResetExtents();
  ...
  // Motion functions
    PSAniKey GetAniKey(int state, int obj, int frame)
        { return &(objects[obj].anikeys[state][frame]); }
...
#define ANI3D_CACHEEXECUTEBUFFER    8   // (default TRUE) caches obj exbuf if not using DrawPrimitive
...
class T3DAnimator : public TObjectAnimator
{
  ...
  // Render functions   
    virtual void RefreshZBuffer()
      ...
      // Called for all imagery before rendering to refresh the z buffer behind the object
    void DrawShadow()
      { ((PT3DImagery)image)->DrawShadow(inst); }
      // Called to draw shadow
    void PreRender();
  ...
  // Extents crap
    ...
    void RecordNewExtents(PTObjectInstance oi, int state = -1);
