...
class TActionBlock
{
  ...
  public:
	...
	~TActionBlock() { /*if (data) free(data);*/ } // VERY BIG HACK SO SPELLBLCKS DON'T DELETE WHEN ACTION BLOCKS DELETE
	...
	int angle, moveangle;		// Angle of movement
...
class TComplexObject : public TObjectInstance
{
  public:
	...
	BOOL IsDoing(char *state) { return doing ? doing->Is(state) : FALSE; }
	...
	BOOL IsRoot(char *state) { return root ? root->Is(state) : FALSE; }
	    // Returns true if the desired state matches the given state
	BOOL IsDoing(ACTION action) { return doing ? doing->action == action : FALSE; }
	    // Returns true if currently doing the given state
