...
// hack alert - z vals are going nuts, so here's a quick fix
#define FIX_Z_VALUE(z)                  ((float)z / ((float)1.46 - ((float)0.01 * ((float)z / (float)300.0))))

#define REV_FIX_Z_VALUE(z)				((float)z + 100.0f)
...
#define OF_ICED			 (1<<22)	// Character is iced (frozen)
...
#define OF_FIXEDFLAGS (OF_ANIMATE | OF_PULSE | OF_COMPLEX | OF_NOTIFY | OF_NONMAP | OF_AI | OF_MOVING)
...
#define OBJFLAGNAMES { "IMMOBILE", "EDITORLOCK", "LIGHT", "MOVING", "ANIMATING", "AI", "DISABLED",\
			...
			"ANIMATE", "PULSE", "WEIGHTLESS", "COMPLEX", "NOTIFY", "NONMAP", "ONEXIT", "PAUSE"}
...
class SObjectDef
{
  ...
  union
  {
	...
	BYTE     state;		// State of object
	BYTE	 group;		// Group of object
	BYTE	 level;		// Level of object
};
...
class TObjectInstance : protected SObjectDef
{
  ...
  // Object status functions
	...
	BYTE GetPrevState() { return prevstate; }
	...
	void SetFrame(int newframe) { prevframe = frame; frame = newframe; }
		// Sets frame number
  ...
  // Object flag functions 	
	...
	void SetFlags(DWORD newflags, DWORD newmask = 0xFFFFFFFF)
		{ flags |= newflags & newmask; }
		// Sets object flags
	void SetFlag(DWORD newflag, BOOL on = TRUE)
		{ if (on) flags |= newflag; else flags &= ~newflag; }
  ...
  // Rotation and velocity functions
  ...
  // Object drawing functions (Called by MapPane only.. don't call directly)
	virtual BOOL OnObject(SPoint &p)
	  { SRect r; imagery->GetScreenRect(this, r); return r.In(p); }
  ...
  // More spiffy virtuals
	...
	virtual DWORD Move();
		// Cause object to make next time unit of movement based on facing and speed (no drawing done here)
	virtual void Animate(BOOL draw);
		// Causes the object to prepare the next frame of its animation
  ...
  // Nextmove functions
	void SetNextMove(RS3DPoint p) { nextmove = p; }
	  // Set next frame's movement.
	void GetNextMove(RS3DPoint p) { p = nextmove; }
	  // Get next frame's movement.
	void Halt() { memset(&nextmove, 0, sizeof(S3DPoint)); }
  ...
  // in the local object should not be defined here, but only in that object.
	...
	//POBJSTATFUNC(TypeFlags)
	POBJSTATFUNC(Agressive)
  ...
  //Animation/drawing
	...
	short prevframe;			// Previous frame
  ...
  // Working members
	S3DPoint			nextmove;	// Set by animator
	int					moveangle;	// Used with nextmove
