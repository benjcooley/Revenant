...
BOOL T3DImagery::CalcObjectMatrixCopy(PS3DAnimObj animobj, int state, int frame, LPD3DMATRIX pos, BOOL calcparents)
{
    int objnum = animobj->objnum;
    PS3DObj obj = &objects[objnum];
    D3DMATRIX m;
    
    memcpy(&m, &animobj->matrix, sizeof(D3DMATRIX));
...
void T3DAnimator::RefreshControllers(int newstate)
{
  ...
  // Remove controllers for other states
    ...
    {
        int tagstate = controllers[c]->TagState();
